...
Place in:  verilog_backend\server.py
...
REQUIREMENTS (pip install):
  ...
  groq                 (only if using Groq)
  anthropic            (only if using Anthropic)

SYSTEM TOOLS NEEDED ON PATH:
  iverilog, vvp   — Icarus Verilog (https://bleyer.org/icarus/ on Windows)
  yosys           — optional, only needed for /synthesize
"""
...
if _GEMINI_KEY:
    try:
        ...
        _ai_client   = _genai.Client(api_key=_GEMINI_KEY)
        _ai_provider = "gemini"
...
# CORS: allow the local dev frontend (file:// or http://localhost:*) plus
# the deployed Render frontend if you also use that. "*" is simplest for
# local dev — tighten this if you ever expose the backend publicly.
app.add_middleware(
    CORSMiddleware,
...
def compile_code(req: CompileReq):
    ...
    try:
        ...
        of  = os.path.join(tmp, "out.vvp")
        tbf = None
        Path(df).write_text(req.design, encoding="utf-8")
        files = [df]

        if req.testbench and req.testbench.strip():
            tbf = os.path.join(tmp, "testbench.v")
            Path(tbf).write_text(req.testbench, encoding="utf-8")
            files.append(tbf)

        r = subprocess.run(
            [IVERILOG, *lang_flags(req.language), "-o", of, *files],
            capture_output=True, text=True, timeout=TIMEOUT
        )
        raw  = r.stderr + r.stdout
        errs = parse_errors(raw, df, tbf)
        errs += _static_analysis(req.design, "design.v")
        if req.testbench:
            errs += _static_analysis(req.testbench, "testbench.v")
        ...
        return {"success": False,
                "errors": [{"file":"compiler","line":0,"severity":"error",
...
def synthesize(req: SynthReq):
    if not shutil.which("yosys"):
        raise HTTPException(
            503,
            "Yosys is not installed on this backend. Install it (e.g. "
            "apt-get install -y yosys on Linux, or via the Yosys Windows "
            "installer) to enable gate-level synthesis."
        )
